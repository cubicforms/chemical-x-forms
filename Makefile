...
watch-bg:  ## Detached watcher (PID tracked in /tmp/attaform-watch.pid) — used by attaform' make link-attaform
	...
	@docker compose exec -e CI=true -e SHELL=/bin/sh -d cx sh -c 'if [ -f /tmp/attaform-watch.pid ]; then PID=$$(cat /tmp/attaform-watch.pid); if [ -f /proc/$$PID/cmdline ] && tr "\0" " " < /proc/$$PID/cmdline | grep -q "prepack:watch"; then exit 0; fi; fi; pnpm prepack:watch > /tmp/attaform-watch.log 2>&1 & echo $$! > /tmp/attaform-watch.pid'
...
unwatch:  ## Stop the background watcher started by watch-bg
	...
	@docker compose exec cx sh -c 'if [ -f /tmp/attaform-watch.pid ]; then PID=$$(cat /tmp/attaform-watch.pid); if [ -f /proc/$$PID/cmdline ] && tr "\0" " " < /proc/$$PID/cmdline | grep -q "prepack:watch"; then pkill -P $$PID 2>/dev/null || true; kill $$PID 2>/dev/null || true; fi; rm -f /tmp/attaform-watch.pid /tmp/attaform-watch.log; fi; true'
