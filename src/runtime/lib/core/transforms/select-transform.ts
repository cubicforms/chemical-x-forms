...
function generateEqualityExpression(
  ...
  function getExpressionNodeChildren(
    ...
    if (multipleExpression === undefined) { // this should NEVER happen
      throw new Error("Programming error: `multiple` expression for `select` node not generated while transforming AST")
    }
  ...
  return getExpressionNodeChildren(
    selectValue,
    optionValue,
...
function extractMultipleFromSelectSummarizedProps(
  ...
  const multipleAttributeIndex = props.findIndex(
    prop => prop.key === "multiple",
  )
...
export const selectNodeTransform: NodeTransform = (node) => {
  const isSelect = node.type === 1 && node.tag === "select"
  ...
  const multipleExpression
        = extractMultipleFromSelectSummarizedProps(selectProps)

  const previousOptionExpressions: CompoundExpressionNode["children"][]
        = typeof multipleExpression === "string"
          ? [[multipleExpression]]
          : [multipleExpression]
