...
- A form accessed only by `injectForm(key)` stays alive as long
...
- **No ambient form** — you called `injectForm()` with no
  ancestor `useForm` and no key argument. The error names both
  resolutions so you can pick either.
- **Key not registered** — you called `injectForm('key-name')`
