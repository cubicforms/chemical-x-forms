...
describe('parseApiErrors (structured result)', () => {
  ...
  describe('envelope shapes', () => {
    ...
    it('unwraps the bare { details } envelope', () => {
      const result = parseApiErrors({ details: { email: ['Email already in use'] } }, { formKey })
      expect(result.ok).toBe(true)
      expect(result.errors).toEqual([{ message: 'Email already in use', path: ['email'], formKey }])
    })

    it('accepts a raw details record', () => {
      const result = parseApiErrors({ email: ['Email already in use'] }, { formKey })
      expect(result.ok).toBe(true)
      expect(result.errors).toEqual([{ message: 'Email already in use', path: ['email'], formKey }])
    })
  ...
  describe('value normalisation', () => {
    ...
    it('splits dotted paths into structured segments', () => {
      const result = parseApiErrors({ details: { 'address.line1': ['required'] } }, { formKey })
    ...
    it('normalises integer-like path segments to numbers', () => {
      const result = parseApiErrors({ details: { 'items.0.name': ['required'] } }, { formKey })
