...
Each anonymous `useForm` call resolves to its own `FormMeta`, so
...
`injectForm<Form>()` (ambient mode) regardless of whether the
form has a key.
...
- **`attaform/zod`** — zod v4 (recommended for new projects)
- **`attaform/zod-v3`** — zod v3 (legacy)

If you were already importing from `/zod-v3`, nothing changes. If
you were importing from the bare `attaform` entry and
...
Pass `createAttaform({ devtools: false })` to disable.
...
```ts
const { register, fieldErrors } = injectForm<Form>()
```
...
`attaform` now ships `"sideEffects": false` in its
...
[`CONTRIBUTING.md`](https://github.com/attaform/attaform/blob/main/CONTRIBUTING.md#releasing-maintainers-only).
